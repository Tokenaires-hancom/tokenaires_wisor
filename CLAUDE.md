...
4명 다 비개발자라 코드를 읽고 판단할 수 없습니다. 그래서 "읽기 좋음"이 아니라 "검증 가능함"을 기준으로 삼습니다. 배경은 `docs/pr-review-system/01-기획서.md` 참고.

- **파급범위 최소화.** 한 기능 변경이 건드리는 파일 수·모듈 경계를 최소화합니다. AI에게 한 부분을 고치라고 시켰을 때 다른 곳이 깨지지 않도록, 모듈을 작게 서로 독립적으로 유지합니다.
- **관용적 패턴 우선.** 똑똑한 트릭이나 새 추상화보다 예측 가능한 흔한 패턴을 씁니다. AI가 다시 이 코드를 이해하고 고칠 확률을 높이는 게 목적입니다.
- **중복 없이 재사용.** 같은 로직을 복붙하지 말고 함수로 뽑아 재사용합니다. 문자 그대로 동일한 코드 블록이 두 곳에 있으면 커밋 자체가 로컬에서 막힙니다(`.githooks/pre-commit`). 이름·구현이 달라도 하는 일이 같은 "의미상 중복"은 여기 원칙으로 판단합니다.
- **커밋은 왜를 설명.** "브랜치와 커밋" 절의 형식 규칙을 그대로 따르되, 다음에 AI에게 다시 시킬 때 맥락을 넣어줄 수 있도록 왜 이렇게 했는지를 남깁니다.

`git push` 전에 `/code-review`와 `/simplify`가 이 섹션을 포함한 루트 `CLAUDE.md` 전체를 감사 근거로 자동 실행됩니다("Claude Code 사용 규칙" 참고). 발견 사항이 있으면 고칠지 물어보고, 병합을 막지는 않습니다 — 판단이 필요한 위반은 항상 사람이 최종 결정합니다.
...
PR마다 자동검사가 제목·본문 형식을 확인하고 코멘트를 남깁니다. **제목·본문 형식은 병합을 막지 않습니다** — 사람이 코멘트를 보고 고칠지 그대로 둘지 정합니다. 다만 같은 자동검사의 테스트·빌드·타입체크·경계 검사는 branch protection에 필수 상태 검사로 등록돼 있어 **실패하면 병합 자체가 막힙니다.**
...
**커밋 전 — 로컬 하드블록**
`git commit`을 시도하면 변경된 영역의 테스트와 문자 그대로의 중복 코드를 자동으로 검사합니다(`.claude/hooks/require-review-before-commit.sh`, 터미널 직접 커밋 시엔 `.githooks/pre-commit`이 대신 걸립니다). 실패하면 커밋이 막히고, 2회까지 자동으로 고쳐봅니다(테스트 파일은 건드리지 않습니다). 그래도 안 되면 사유를 남기고 오버라이드로 강행할 수 있지만, 다음 push 시 PR에 흔적이 남습니다.
