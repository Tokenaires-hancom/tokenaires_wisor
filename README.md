...
투자 대가의 판단 기준으로 종목을 좁히고, 확인한 것을 하나의 학습노트로 모읍니다.

**동작하는 서비스** — <https://wisor.site>. Web과 해설 챗봇은 OCI의 별도 컨테이너이며
Nginx가 하나의 주소로 연결합니다. `main` 애플리케이션 변경은 검사 후 커밋 SHA 단위로
자동 배포됩니다. 운영 계약은 `docs/oci-autodeploy.md`, 기존 Render 설정은 `render.yaml`에
남아 있습니다.
...
| 해설 챗봇 | 대가 페르소나로 지표를 해설. 금지표현 필터와 재생성. OCI 배포·연결 완료 |
| 데이터 | S&P 500·NASDAQ-100 중복 제거 유니버스 517개 중 품질 기준을 통과한 380개 종목 수록 |
| Supabase | 회원 계정 저장·비회원 임시 기록 자동 이전 구현. 원격 마이그레이션 적용 필요 |
...
**사용자 데이터 저장은 한 파일을 거칩니다.** `lib/store.ts`가 비회원에게는 브라우저 임시 저장을,
회원에게는 사용자별 RLS가 적용된 Supabase 저장을 사용합니다. 인증 직후 임시 기록은 계정의
기존 기록과 병합되고, 성공한 뒤에만 브라우저에서 제거됩니다.
...
- [ ] Supabase 연결 배포 — 앱 구현 완료, 원격 프로젝트에 증분 SQL 적용과 테스트 계정 검증 필요
