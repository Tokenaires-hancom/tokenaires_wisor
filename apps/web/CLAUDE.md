...
Tailwind, CSS-in-JS, UI 라이브러리를 쓰지 않습니다. 스타일은 `app/styles/`의 토큰과 클래스만 씁니다. 인라인 `style`은 일회성 여백 조정에만 허용합니다.

### 새 규칙을 어느 파일에 넣나

`app/styles/`는 12개 파일이고 **화면 단위로 나뉘어 있습니다.** 어느 파일이 무엇을 담았는지는 `docs/css-split-check.md`의 표에 있습니다.

- 이미 있는 화면을 고치면 → 그 화면의 파일 (`/me`면 `me.css`)
- 여러 화면이 함께 쓰는 것이면 → `base.css`, 색·간격 토큰이면 `tokens.css`
- **새 화면을 만들면 새 파일을 만듭니다.** 아무 파일 끝에 붙이지 않습니다 — 그렇게 커진 결과가 5,316줄짜리 `globals.css`였습니다

새 파일은 `layout.tsx`의 import 목록 **맨 아래**에 넣습니다. 중간에 끼우면 뒤 규칙이 앞 규칙을 덮는 관계가 바뀝니다. 12개의 순서는 원래 한 파일이었을 때의 순서 그대로라 바꾸면 안 됩니다.
...
색은 `app/styles/tokens.css`의 CSS 변수만 씁니다. 하드코딩한 hex를 새로 넣지 않습니다.
