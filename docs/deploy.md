...
### 서버 환경변수 (`/etc/wisor/web.env`)

| 키 | 값 | 없으면 |
|---|---|---|
| `SITE_ORIGIN` | `https://wisor.site` | `http://localhost:3000`으로 떨어져 **계정 삭제가 전부 403**입니다. 브라우저가 보내는 `Origin`과 대조하는 값이라, 공개 주소와 다르면 아무도 계정을 지울 수 없습니다 |
...
> 않으므로, 남은 연결 여부는 Netlify 대시보드에서 확인하고 필요하면 해제하세요. — 백엔드 담당
...
`.github/workflows/scores.yml`은 **지금 동작하지 않습니다.** 토스 API가 송신 IP를 제한하는데
허용 목록에는 OCI 서버 주소만 있고, GitHub 호스티드 러너는 실행마다 다른 주소로 나갑니다.
그래서 `--provider sec-toss`가 토큰 발급 단계에서 `HTTP 403`으로 막힙니다. 2026-08-26 시점
예약 실행 20건이 전부 403으로 끝난 것도 같은 이유입니다.

**secret을 새로 발급해도 살아나지 않습니다.** 자격증명이 아니라 출발 주소가 막힌 것이라,
살리려면 서버와 같은 주소에서 실행해야 합니다(자체 호스티드 러너). 허용 목록을 러너까지
넓히는 쪽은 주소가 매번 바뀌므로 불가능합니다.

그래서 **운영 데이터 갱신 경로는 위의 서버 배치 하나뿐이고, 배치가 막혔을 때 대신 쓸 수단은
지금 없습니다.** 이 워크플로를 되살릴지 이름과 용도를 바로잡을지는 아직 정해지지 않았습니다.
