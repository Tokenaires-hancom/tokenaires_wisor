...
| 브랜치 | 보호 | 뜻 |
|---|---|---|
| `develop` | `check` 통과 필수 · 최신 상태 필수 · **승인 1명** · 코드오너 리뷰 | 리뷰 없이는 병합 버튼이 잠깁니다 |
| `main` | `check` 통과 필수 · 최신 상태 필수 · PR 필수 (승인은 요구하지 않음) | 검사가 빨간 X면 잠깁니다. 혼자서도 승격할 수 있습니다 |

둘 다 강제 push와 브랜치 삭제를 막습니다. admin은 급할 때 우회할 수 있게 열어 뒀습니다.

`main`은 2026-08-31까지 아무 보호가 없었고, 그래서 차단 사유가 적힌 PR이 그대로 병합돼
배포가 실패한 적이 있습니다(PR #80). 같은 날 위 규칙을 걸었습니다.

**`main`에 직접 push할 수 없게 됐습니다.** [scores.yml](../.github/workflows/scores.yml)을
수동으로 돌릴 때는 `main`이 아니라 `develop`이나 `data/` 브랜치를 골라 실행한 뒤 승격합니다.
운영 데이터를 만드는 OCI 배치는 git을 쓰지 않으므로 영향이 없습니다.
