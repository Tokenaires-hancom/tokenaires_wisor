...
**받아들인 리스크**: ②는 로컬 세션에서만 돈다 — `claude-rules-review.yml`처럼 GitHub 이벤트에 무조건 걸리는 서버 트리거가 없어서, 터미널 직접 push나 세션 중단 시 아예 안 돌 수 있다. 저장소 커밋 기록으로도 "Claude Code를 거쳤는지" 사후 확인이 원래 불가능했다는 점(공동저자 트레일러 관행 없음)을 근거로 naemnaem99가 이 리스크를 수용했다.
...
flowchart LR
    subgraph 경로별 소유자 [".github/CODEOWNERS"]
        Root["* → 4명 전원"]
        Sensitive[".claude/**, .githooks/**,<br/>CLAUDE.md, scripts/pr_checks/**<br/>→ naemnaem99"]
    end
    ...
    subgraph 병합 시도
        ...
        Check3 -- "예, naemnaem99 미승인" --> Block
        Check3 -- "아니오 또는 naemnaem99 승인함" --> Allow(["병합 가능"])
    end
```
...
- 오버라이드 흔적은 경로로 못 잡으므로, `pr-review.yml`에 "PR에 오버라이드 마커가 있으면 naemnaem99를 리뷰어로 지정" 하는 작은 스텝을 추가한다.
