...
투자 대가 페르소나로 재무 지표를 **해설**하고 투자 철학을 자유롭게 묻는 챗봇.
종목을 고르면 팀 `scores.json`의 숫자만 해설하고, 고르지 않으면 특정 회사를 가정하지 않는다.
...
프론트는 지표 값을 보내지 않는다. 종목 대화는 `{ticker, persona}`, 자유 대화는
`{persona}`만 보낸다.
...
종목을 고르지 않으면 대가 종류와 무관하게 자유 대화 앵커를 쓴다. 세션 생성과 대가
전환은 고정 안내문이라 모델을 부르지 않는다. 첫 실제 질문은 정상 응답이면 한 번,
빈 응답을 재시도하면 최대 두 번 모델을 호출한다.

```
[system]    자유 대화 규칙 + 대가 프롬프트
[user]      <대화맥락>                ← 특정 회사·수치를 가정하지 않는 앵커
[assistant] 고정 안내문
[user]      첫 실제 질문 …
```
...
- 대가는 1인칭으로 말한다
- 자유 대화는 대가 본문에 적힌 투자 기준 수치는 설명할 수 있지만, 선택하지 않은
  회사의 실제 수치·현재 시장 수치·정확한 인용을 지어내지 않는다
...
- 자유 대화 세션 생성은 분당 60개로 제한해 빈 세션이 활성 대화를 밀어내지 못하게 한다
